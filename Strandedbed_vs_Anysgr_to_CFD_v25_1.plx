...
# Rewrite : Nick Kent 5th Dec 2025 - Changes input to stranded Rossi et al. format .bed file rather than .site. This
# allows use with mix of .sgr inputs without requiring a full PartN series like Rossistrandedbed_vs...plx.

# USAGE:- perl Strandedbed_vs_Anysgr_to_CFD_v24_2.plx
#
# FUNCTION:
# This script takes .bed files containing a list of sites/genomic features such as TSSs or TF  
...
# The input .bed files should have 6 columns: chrn; site pos start; site pos end; summary; ChEx score; Strand F or R
#
...
# 
# The _CFD_matrix.txt file writes bins and then all sgr-labelled/specific CFD values in simple horizontal rows and 
...
# $siteID_indir_path - The directory containing the site list .bed file
...
foreach $infile_siteID (@files_siteID){    
    ...
    # ignore hidden files and only get those ending .bed (added end-of-string anchor)
    if (($infile_siteID !~ /^\.+/) && ($infile_siteID =~ /.*\.bed\z/)){
	...
	# loop through infile to get values
	    ...
	    # split line by delimiter and store elements in an array
			...
			#altered here to force numeric/string
				...
				print "Found an unexpected data line in the .bed; trying to skip, but you might want to check the file.\n";
