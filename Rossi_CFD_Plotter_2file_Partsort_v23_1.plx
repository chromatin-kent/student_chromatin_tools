...
# Modified: Nick Kent 10th Dec 2023 - sorts the .sgr filnames according to ascending PartN to
# place PartN data in numerical order in the matrix file.

# USAGE:- perl Rossi_CFD_Plotter_2file_Partsort_v23_1.plx
#
# FUNCTION:
# This script takes .bed files containing a list of sites/genomic features (these
# traditionally are TSSs or TF sites, but in this script version should be  Rossi et al., 2021
# ChIP-Exo_seq factor positions) and compares it with 
...
# The input .bed files should have 6 columns: chrn; site pos start; site pos end; summary; ChEx score; dot.
#
...
# 
# The _CFD_.txt file writes bins and then all PartN-specific CFD values in simple horizontal rows and 
	...
	# loop through infile to get values
	    ...
	    # split line by delimiter and store elements in an array
			...
			#altered here to force numeric/string - with Rossi data this should fail to match anything
				...
				print "Found an unexpected data line in the Rossi .bed file; trying to skip, but you might want to check the file.\n";
...
# Add numerical sort to input file names.
@sorted_files_sgr = sort { (($a =~ /Part(\d+)/)[0] || 0) <=> (($b =~ /Part(\d+)/)[0] || 0) } @files_sgr;
