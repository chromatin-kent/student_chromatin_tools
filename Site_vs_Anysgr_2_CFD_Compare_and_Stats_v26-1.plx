...
# USAGE:- perl Site_vs_Anysgr_2_CFD_Compare_Stats_vxxx.plx
#
...
#
# The input .site file should have four columns: chrn; Site ID; site dyad pos; strand.
#
...
my $descriptor="WT_vs_chd1_SUB101test";
my @particles = ("Part150"); #comma delimit
	...
	# loop through infile to get values
	    ...
	    # split line by delimiter and store elements in an array
			...
			if($line_siteID[3] =~ "F") { # infile if 1
				...
				push(@site_pos,$line_siteID[2]);
				push(@site_strand,$line_siteID[3]);
			
			}elsif ($line_siteID[3] =~ "R"){ # infile elsif 1
